...
db_path = os.path.join(data_dir, "db/eq_data.db")
...
# Read the text file
txt_path = os.path.join(data_dir, "text/eqstr_us.txt")
try:
...
for match in matches:
    id_num, text = match
    cursor.execute(
        "INSERT OR REPLACE INTO eqstr_us (id, text) VALUES (?, ?)", (int(id_num), text)
    )
